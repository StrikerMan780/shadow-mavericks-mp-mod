...
vec4 Process(vec4 color)
{
	...
	vec2 n0Uv = vec2(uv.x*1.4 + 0.01, uv.y + timer*0.69);
	vec2 n1Uv = vec2(uv.x*0.5 - 0.033, uv.y*2.0 + timer*0.12);
	vec2 n2Uv = vec2(uv.x*0.94 + 0.02, uv.y*3.0 + timer*0.61);
	...
	float n0 = (getBaseTexel(n0Uv).w-0.5)*2.0;
	float n1 = (getBaseTexel(n1Uv).w-0.5)*2.0;
	float n2 = (getBaseTexel(n2Uv).w-0.5)*2.0;
	float noiseA = clamp(n0 + n1 + n2, -1.0, 1.0);

	// Generate noisy y value
	vec2 n0UvB = vec2(uv.x*0.7 - 0.01, uv.y + timer*0.27);
	vec2 n1UvB = vec2(uv.x*0.45 + 0.033, uv.y*1.9 + timer*0.61);
	vec2 n2UvB = vec2(uv.x*0.8 - 0.02, uv.y*2.5 + timer*0.51);
	...
	float n0B = (getBaseTexel(n0UvB).w-0.5)*2.0;
	float n1B = (getBaseTexel(n1UvB).w-0.5)*2.0;
	float n2B = (getBaseTexel(n2UvB).w-0.5)*2.0;
	...
	vec4 flamecolor = getBaseTexel(finalNoise);
